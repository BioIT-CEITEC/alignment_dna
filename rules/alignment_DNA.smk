...
def mark_duplicates_ref(wildcards):
    ...
    if config["umi_usage"] == "umi_concensus":
        input['ref'] = BR.remote(expand("{ref_dir}/index/BWA/{ref}.bwt",ref_dir=reference_directory,ref=config["reference"])),
...
rule alignment_DNA:
    input:  fastqs = alignment_DNA_input,
            ref = BR.remote(expand("{ref_dir}/index/BWA/{ref}.bwt",ref_dir=reference_directory,ref = config["reference"])),
